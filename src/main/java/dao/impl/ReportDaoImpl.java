...
public class ReportDaoImpl extends AbstractCrudDao<ReportEntity> implements ReportDao {
    ...
    public List<ReportEntity> findByUser(Long userId, Integer currentPage, Integer recordsPerPage) {
        try (Connection connection = connector.getConnection();
            ...
            LOGGER.error("Invalid insertion report", e);
            throw new DataBaseRuntimeException("Invalid insertion report", e);
        }
    }

    @Override
    public Integer countRowsForUserById(Long userId) {
        return getRowCountForLongParam(userId, COUNT_QUERY_FOR_USER);
    }
